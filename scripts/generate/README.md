# `generate/` — enrich component schemas with upstream transform types

> **Pipeline stage 2** (see [`scripts/README.md`](../README.md)). `generate_schemas.go`.
>
> | | |
> |---|---|
...
> **Note:** this stage only *augments* the `transform` block. The component's own
> `inputProperties`/`properties` are authored by hand (or, for site components,
> generated by [`generate-site-schemas/`](../generate-site-schemas/README.md)).
